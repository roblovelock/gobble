...
func TestMany0(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:      args{p: character.Rune('a'), input: strings.NewReader("")},
			wantMatch: []rune{},
		},
		{
			name:       "rune mismatch => no match",
			args:       args{p: character.Rune('a'), input: strings.NewReader("b")},
			wantMatch:  []rune{},
			wantRemain: "b",
		},
		{
			name:      "rune match => match",
			args:      args{p: character.Rune('a'), input: strings.NewReader("a")},
			wantMatch: []rune{'a'},
		},
		{
			name:      "rune match many => match",
			args:      args{p: character.Rune('a'), input: strings.NewReader("aaaa")},
			wantMatch: []rune{'a', 'a', 'a', 'a'},
		},
		{
			name:       "runes match unicode many => match",
			args:       args{p: character.Rune('😀'), input: strings.NewReader("😀😀a")},
			wantMatch:  []rune{'😀', '😀'},
			wantRemain: "a",
		},
	}
